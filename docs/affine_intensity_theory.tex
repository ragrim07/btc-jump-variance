...
% A self-contained mathematical development, written as the implementation
% contract for the Step-A' extension of this repository.
%
...
\newtheorem{impact}[theorem]{Project impact}
...
\large A complete mathematical development: from point-process foundations\\
to GMM moments, filtering, and variance-risk-premium analysis}
\author{Trading / Todorov SDE \& Volatility Calibration project}
\date{July 2026 --- Step A$'$ implementation contract}
...
\begin{abstract}
\noindent
The Step-A calibration of the Todorov (2010) two-factor model was rejected by
three held-out diagnostics: the realized jump-variance series $\JV_t$ is
strongly serially correlated (the model demands independence), the
cross-autocovariance between $\TPV_t$ and $\JV_t$ is two-sided (the model
demands strict one-sidedness), and the jump feed-through persists for about a
week (the calibrated decay dies in hours). All three failures share one cause:
the model's jumps arrive at \emph{constant} intensity. This document develops,
from first principles, the minimal repair: an \emph{affine stochastic
intensity} $\lambda(t)=\lambda_{0}+\lambda_{1}\Vj(t^{-})$, which makes the jump
process self-exciting while preserving closed-form moments. We build the
point-process machinery from scratch, prove why the affine class is the unique
tractable choice, establish the equivalence with an exponential-kernel marked
Hawkes process, derive every population moment the GMM estimator needs using
Dynkin's formula over fixed time intervals, analyze identification, and trace
the consequences through every layer of the project: estimation, the
latent-state filter, variance-risk-premium construction, regime
classification, and the simulation harness that must verify the algebra before
any estimation is run.
\end{abstract}
...
Chapter~\ref{ch:impl} specifies simulation, verification, and the concrete
changes to \texttt{test.py}. Appendices collect derivatives for the analytic
Jacobian, useful integrals, notation, and an implementation checklist.

\paragraph{Mathematical house rules.} Per the project's estimation protocol,
all derivations proceed through \emph{Dynkin's formula evaluated over a fixed
global time interval}. We never shrink a time step to zero as a derivation
device and we never invoke the adjoint (forward/Fokker--Planck) operator.
...
\begin{impact}
Proposition~\ref{prop:closure} is why the whole project works. Moment
...
no analytic Jacobian --- the exact failure modes the project's coding
standards prohibit). Affinity is not a stylistic choice; it is the boundary
between ``rederive formulas in an afternoon'' and ``rebuild the estimator.''
\end{impact}
...
The Step-A transience mandate (``jump factor half-life under 2 days'')
...
in its persistence band as mandated. Two structural constraints deserve
emphasis:
\begin{itemize}
...
\item \textbf{The old transience mandate moves to $b$.} ``Jump factor
...
Chapter~\ref{ch:gmm}. This dependency chain is why estimation rigor comes
before any VRP claim.
\end{impact}
...
state count by held-out likelihood, not in-sample BIC alone (545 daily
observations). Validate against the known event: 2025-10-10 must be flagged
without being told.
...
\begin{impact}
Deliverables enabled by this chapter, in order: the VRP time series
...
an event not targeted by any moment is the strongest single validation
...
\section{The verification protocol (non-negotiable)}
...
\begin{tabular}{lll}
\toprule
component & Step A (\texttt{test.py}) & Step A$'$\\
\midrule
...
weighting & Ledoit--Wolf + ridge & unchanged (more shrinkage)\\
...
Table~\ref{tab:dictionary} is the parameter dictionary; keep it in the
repository documentation and in docstrings (the coding standard requires
each code block to cite its SDE equivalent --- cite equations of this
document by number).
